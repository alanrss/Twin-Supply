// api/paypal-capture-order.js (Vercel Serverless Function - Node.js)
// Captures a PayPal order on the server, optionally decrements stock on GitHub, and sends emails.
...
module.exports = async (req, res) => {
  ...
  function buildEmailHtml({ orderId, customer, totals, items, payment }) {
    ...
    return `
      <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;">
        ...
        <p><strong>Payment:</strong> ${htmlEscape(payment?.provider || "paypal")} ${payment?.method ? `(${htmlEscape(payment.method)})` : ""}</p>
  ...
  try {
    ...
    // Optional safety check: read the order amount before capture
    try {
      const getOrder = await fetch(`${baseUrl}/v2/checkout/orders/${encodeURIComponent(orderID)}`, {
        headers: { Authorization: `Bearer ${accessToken}` }
      });
      const orderData = await getOrder.json().catch(() => ({}));
      const ppAmount = Number(orderData?.purchase_units?.[0]?.amount?.value || 0);
      if (Number.isFinite(ppAmount) && ppAmount > 0) {
        if (Math.abs(round2(ppAmount) - totals.total) > 0.01) {
          return res.status(400).json({
            error: "PayPal order amount mismatch",
            serverTotal: totals.total,
            paypalTotal: round2(ppAmount)
          });
        }
      }
    } catch (_) {
      // non-blocking; capture may still succeed
    }

    const capResp = await fetch(`${baseUrl}/v2/checkout/orders/${encodeURIComponent(orderID)}/capture`, {
      method: "POST",
      headers: {
    ...
    if (!capResp.ok) {
      return res.status(500).json({ error: "PayPal capture failed", detail: capData });
    }
    ...
    const payment = {
      ...
      paypalStatus: capData?.status || "",
      payerEmail: capData?.payer?.email_address || ""
    };
    ...
    try {
      if (adminEmail) {
        await sendEmailIfConfigured({
          to: adminEmail,
          subject: `New Order ${orderId} (${payment.method})`,
          html: emailHtml
        });
      }
      const custEmail = String(customer.email || payment.payerEmail || "").trim();
