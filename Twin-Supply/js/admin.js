...
function readForm() {
  return {
    ...
    department: ($("p-department")?.value || "Clothing").trim(),
...
function fillForm(p) {
  ...
  if ($("p-department")) $("p-department").value = p.department || "Clothing";
...
function clearForm() {
  ...
  $("p-price").value = "";
  $("p-image").value = "";
  if ($("p-department")) $("p-department").value = "Clothing";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Sync (products.json)
  $("export-products")?.addEventListener("click", (e) => {
    e.preventDefault();
    try {
      const json = window.exportProductsJSON ? window.exportProductsJSON() : JSON.stringify(window.allProducts || [], null, 2);
  ...
  const importInput = $("import-products-file");
  importInput?.addEventListener("change", async () => {
    const file = importInput.files && importInput.files[0];
    if (!file) return;
    try {
      const text = await file.text();
      if (window.importProductsJSON) window.importProductsJSON(text);
      renderList();
      alert("Imported. This is now a DRAFT on this device. Export and upload to GitHub to sync everywhere.");
    } catch (err) {
  ...
  $("clear-draft")?.addEventListener("click", async (e) => {
    e.preventDefault();
    if (!confirm("Clear draft and use the live products.json?")) return;
    if (window.clearDraftProducts) window.clearDraftProducts();
    if (window.refreshProductsFromLive) {
      try { await window.refreshProductsFromLive(); } catch {}
    }
    renderList();
  });
